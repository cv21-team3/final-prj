...
class Inpainter():
    ...
    def inpaint(self):
        ...
        while keep_going:
            ...
            imsave('output.jpg',self.working_image, quality=100)
    ...
    def _plot_image(self):
        ...
        imsave('output_raw.jpg',image, quality=100)
    ...
    def _update_T_s(self):
      self.T_s=18*np.count_nonzero(self._calc_normal_matrix)/self.patch_size
    ...
    def _update_priority(self):
        ...
        self._update_K()
        if(self.T_s>self.T):
          self.priority=self.alpha*self.data*self.front+self.beta*(np.abs(self.K[:,:,0])+np.abs(self.K[:,:,1])/2)*self.front
        else:
          self.priority = self.confidence*self.front
    ...
    def _update_K(self):
        ...
        K=gradient/np.abs(gradient)
        K=(K[:,:,0]+K[:,:,1])/2
        print(K.shape)
        x_kernel = np.array([[.25, 0, -.25], [.5, 0, -.5], [.25, 0, -.25]])
        y_kernel = np.array([[-.25, -.5, -.25], [0, 0, 0], [.25, .5, .25]])

        x_normal = convolve(K.astype(float), x_kernel)
        y_normal = convolve(K.astype(float), y_kernel)
    ...
    def _update_data(self):
        ...
        self.normal_gradient=normal_gradient
        self.data = np.sqrt(
   ...
   #   self.grey_scaled=(self.Image[:,:,0]+self.image[:,:,1]+self.image[:,:,2])/3
   #   x_kernel = np.array([[.25, 0, -.25], [.5, 0, -.5], [.25, 0, -.25]])
   #   y_kernel = np.array([[-.25, -.5, -.25], [0, 0, 0], [.25, .5, .25]])
   #   x_normal = convolve(self.working_mask.astype(float), x_kernel)
   #   y_normal = convolve(self.working_mask.astype(float), y_kernel)
   ...
   #   norm[norm == 0] = 1
   #   unit_normal = normal/norm
   #   var=np.var(unit_normal)
   #   mean=np.mean(unit_normal)
   #   front_positions = np.argwhere(self.front == 1)
   #   for point in front_positions:
   #     patch = self._get_patch(point)
   #   unit_normal[patch[0]]
